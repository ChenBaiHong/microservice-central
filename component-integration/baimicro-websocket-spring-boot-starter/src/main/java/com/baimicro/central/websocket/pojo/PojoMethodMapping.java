 ...
 * @description: 处久化 使用关于 com.baihealth.cloud.websocket.annotation 下注解映射处理
 * @author: baiHoo.chen
 * @create: 2020-04-12
 **/
public class PojoMethodMapping {
    ...
    private final Method onMessageFrame;
    ...
    private final MethodParameter[] onMessageFrameParameters;
    ...
    private final MethodArgumentResolver[] onMessageFrameArgResolvers;
    ...
    public PojoMethodMapping(Class<?> pojoClazz, ApplicationContext context, AbstractBeanFactory beanFactory) throws DeploymentException {
        ...
        while (!currentClazz.equals(Object.class)) {
            ...
            for (Method method : currentClazzMethods) {
                ...
                } else if (method.getAnnotation(OnMessageFrame.class) != null) {
                    checkPublic(method);
                    if (messageFrame == null) {
                        messageFrame = method;
                    } else {
                        if (currentClazz == pojoClazz ||
                                !isMethodOverride(messageFrame, method)) {
                            // Duplicate annotation
                            throw new DeploymentException(
                                    "pojoMethodMapping.duplicateAnnotation onMessage");
                        }
        ...
        if (messageFrame != null && messageFrame.getDeclaringClass() != pojoClazz) {
            if (isOverridenWithoutAnnotation(pojoClazzMethods, messageFrame, OnMessageFrame.class)) {
                messageFrame = null;
            }
        }
        ...
        this.onMessageFrame = messageFrame;
        ...
        onMessageFrameParameters = getParameters(onMessageFrame);
        ...
        onMessageFrameArgResolvers = getResolvers(onMessageFrameParameters);
    ...
    Method getOnMessageFrame() {
        return onMessageFrame;
    }

    Object[] getOnMessageFrameArgs(Channel channel, RequestFrame requestFrame) throws Exception {
        return getMethodArgumentValues(channel, requestFrame, onMessageFrameParameters, onMessageFrameArgResolvers);
    }
    ...
    private List<MethodArgumentResolver> getDefaultResolvers() {
        ...
        resolvers.add(new HttpResponseMethodArgumentResolver());
        resolvers.add(new MessageFrameMethodArgumentResolver());
