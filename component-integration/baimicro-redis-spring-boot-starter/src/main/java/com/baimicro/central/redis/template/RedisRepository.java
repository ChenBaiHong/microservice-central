package com.baimicro.central.redis.template;
 ...
 * @author: chen.baihoo
 * @date: 2020/2/15
 * @Description: TODO  redis 基本操作 可扩展,基本够用了
 * version 0.1
 */
@Slf4j
...
//        this.redisTemplate.setValueSerializer(FASTJSON_SERIALIZER);
//        this.redisTemplate.setHashValueSerializer(FASTJSON_SERIALIZER);
    }
...
//            byte[] values = FASTJSON_SERIALIZER.serialize(value);
...
//                byte[] bValues = FASTJSON_SERIALIZER.serialize(values[i]);
...
//            return FASTJSON_SERIALIZER.deserialize(values);
        });
        log.debug("[redisTemplate redis]取出 缓存  url:{} ", key);
        return resultStr;
    }
...
//                    Object value = FASTJSON_SERIALIZER.deserialize(bValues);
                    maps.put(key, value);
                }
            }
            return maps;
