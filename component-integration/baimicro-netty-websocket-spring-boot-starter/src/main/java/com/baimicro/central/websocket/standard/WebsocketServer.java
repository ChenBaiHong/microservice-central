        ...
        bootstrap.group(boss, worker)
                ...
                .childHandler(new ChannelInitializer<NioSocketChannel>() {
                    ...
                    protected void initChannel(NioSocketChannel ch) throws Exception {
                        ...
                        // 添加自定义 rpcServer 处理handler
                        // ⚠️注意：rpcServiceServerHandler 必须放在 http 请求的处理器之前，因为我们不需要走 http 请求
                        pipeline.addLast("rpcServiceServerHandler", new RpcServiceServerHandler(pojoEndpointServer));
                        /**
                         * 添加关于 http 请求的处理器
                         */
                        pipeline.addLast("httpServerCodec", new HttpServerCodec());
                        pipeline.addLast("httpObjectAggregator", new HttpObjectAggregator(65536));
                        // 添加自定义 httpServer 处理handler
