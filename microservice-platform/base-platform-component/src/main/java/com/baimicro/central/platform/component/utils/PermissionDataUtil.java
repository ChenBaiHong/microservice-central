...
public class PermissionDataUtil {
     ...
     * @param permission
     */
    public static PlatfPermission intelligentProcessData(PlatfPermission permission) {
        if (permission == null) {
            return null;
        }
     ...
     * @param metaList
     */
    public static void addIndexPage(List<PlatfPermission> metaList) {
        boolean hasIndexMenu = false;
        for (PlatfPermission sysPermission : metaList) {
        ...
        if (!hasIndexMenu) {
            metaList.add(0, new PlatfPermission(true));
        }
    }
