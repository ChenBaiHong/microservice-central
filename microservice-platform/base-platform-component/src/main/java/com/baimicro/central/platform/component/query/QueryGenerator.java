package com.baimicro.central.platform.component.query;

import com.alibaba.fastjson.JSON;
import com.baimicro.central.platform.component.utils.ConvertUtils;
import com.baimicro.central.platform.component.utils.DataAutorUtils;
import com.baimicro.central.platform.component.utils.SqlInjectionUtil;
...
public class QueryGenerator {
    ...
    public static void installMplus(QueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMap) {
		...
		*/
        ...
        Map<String, PlatfPermissionDataRule> ruleMap = getRuleMap();
    ...
    public static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap) {
        ...
        if (ConvertUtils.isNotEmpty(column) && ConvertUtils.isNotEmpty(order)) {
            //字典字段，去掉字典翻译文本后缀
            if (column.endsWith(PlatformCommonConstant.DICT_TEXT_SUFFIX)) {
                column = column.substring(0, column.lastIndexOf(PlatformCommonConstant.DICT_TEXT_SUFFIX));
            }
            //SQL注入check
     ...
     * @return
     */
    public static Map<String, PlatfPermissionDataRule> getRuleMap() {
        Map<String, PlatfPermissionDataRule> ruleMap = new HashMap<String, PlatfPermissionDataRule>();
        List<PlatfPermissionDataRule> list = DataAutorUtils.loadDataSearchConditon();
        if (list != null && list.size() > 0) {
            if (list.get(0) == null) {
                return ruleMap;
            }
            for (PlatfPermissionDataRule rule : list) {
    ...
    private static void addRuleToQueryWrapper(PlatfPermissionDataRule dataRule, String name, Class propertyType, QueryWrapper<?> queryWrapper) {
